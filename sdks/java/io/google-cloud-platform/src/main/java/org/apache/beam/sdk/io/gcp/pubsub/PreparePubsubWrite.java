...
public class PreparePubsubWrite<InputT>
  ...
  // Only one of the two below should be set. 
  private SerializableFunction<InputT, PubsubIO.PubsubTopic> topicValueProviderFunction;
  ...
  public PreparePubsubWrite(
      SerializableFunction<InputT, PubsubIO.PubsubTopic> topicValueProviderFunction,
      SerializableFunction<InputT, PubsubMessage> formatFunction) {
    this.topicValueProviderFunction = topicValueProviderFunction;
  ...
  public PCollection<PubsubMessage> expand(PCollection<InputT> input) {
    ...
    return input.apply(ParDo.of(new PreparePubsubWriteDoFn()));
  }
    ...
    public void processElement(ProcessContext context, @Element InputT element) {
      ...
      if (formatFunction == null) {
        ...
        checkArgument(
            ...
            formatFunction,
            element);
        if (outputValue.getTopicPath() == null && topicValueProviderFunction != null) {
          PubsubMessage temp = (PubsubMessage) element;
          outputValue =
              PubsubMessage(
                  temp.getPayload(),
                  temp.getAttributeMap(),
                  temp.getMessageId(),
                  temp.getOrderingKey(),
                  topicValueProviderFunction.apply(element).toString());
        }
        // use static topic instead
