...
public class PubsubUnboundedSink extends PTransform<PCollection<PubsubMessage>, PDone> {
  ...
  /** Get the topic function used to create the topics to write to. */
  public SerializableFunction<PubsubMessage, String> getTopicFn() {
    return topicFn;
  }
      ...
      return input
          .apply(
              ...
              MapElements.into(new TypeDescriptor<KV<String, byte[]>>() {})
                  .via(
                      new PubsubMessages.ParsePayloadAsPubsubMessageProtoWithTopicFn(this.topicFn)))
          .setCoder(KvCoder.of(StringUtf8Coder.of(), ByteArrayCoder.of()))
          .apply(new PubsubSink(this));
    }

    // if topic fn is not passed
    return input
        .apply(
            "Output Serialized PubsubMessage Proto",
            MapElements.into(new TypeDescriptor<KV<String, byte[]>>() {})
                .via(
                    new PubsubMessages.ParsePayloadAsPubsubMessageProtoWithTopic(
                        this.getTopic().getPath())))
        .setCoder(KvCoder.of(StringUtf8Coder.of(), ByteArrayCoder.of()))
        .apply(new PubsubSink(this));
  }
